...
const char *pattern, const size_t pattern_len){
	...
	memcpy(buff_array, f_array + f_array_length - pattern_len + 1, pattern_len - 1);
    memcpy(buff_array + pattern_len - 1, s_array, pattern_len - 1);

    unsigned int pattern_counter = pattern_entity(buff_array, pattern, pattern_len);
    free(buff_array);
...
const size_t array_size, const char *pattern){
	...
	for(size_t c = 0; c < array_size - 1; ++c){
		...
		pattern_counter += pattern_entity_in_slice(array[c],
		array[c + 1], pattern, pattern_length);
	}
	pattern_counter += pattern_entity(array[array_size - 1], pattern, pattern_length);
